...
Let's start with looking at Brazil, our ROI
...
### Chen et al. 2022
First read in projected 2020 cropland from Chen et al. 2022
...
```{r}
# read in rasters with terra::rast
# read in three different SSP scenarios for 2020
chen_ssp1_2020_global <- rast("data/chen/global_PFT_SSP1_RCP26_2020.tif")
#chen_ssp3_2020_global <- rast("data/chen/global_PFT_SSP3_RCP70_2020.tif")
#chen_ssp5_2020_global <- rast("data/chen/global_PFT_SSP5_RCP85_2020.tif")
...
chen_ssp1_2020 <- chen_ssp1_2020_global %>% 
  crop(brazil_trans) %>% 
  mask(brazil_trans) %>% 
  terra::project(y = crs(brazil))

#chen_ssp3_2020 <- chen_ssp3_2020_global %>% 
  #crop(brazil_trans) %>% 
  #mask(brazil_trans) %>% 
  #terra::project(y = crs(brazil))

#chen_ssp5_2020 <- chen_ssp5_2020_global %>% 
  #crop(brazil_trans) %>% 
  #mask(brazil_trans) %>% 
  #terra::project(y = crs(brazil))

# isolate just cropland from each raster
chen_cropland_ssp1_2020 <- chen_ssp1_2020
chen_cropland_ssp1_2020[chen_cropland_ssp1_2020 != 18] <- 0 # if value isn't 18, make 0
chen_cropland_ssp1_2020[chen_cropland_ssp1_2020 == 18] <- 10 # if value is 18, make 10


#chen_cropland_ssp3_2020 <- chen_ssp3_2020
#chen_cropland_ssp3_2020[chen_cropland_ssp3_2020 != 18] <- 0 
#chen_cropland_ssp3_2020[chen_cropland_ssp3_2020 == 18] <- 1 

#chen_cropland_ssp5_2020 <- chen_ssp5_2020
#chen_cropland_ssp5_2020[chen_cropland_ssp5_2020 != 18] <- 0
#chen_cropland_ssp5_2020[chen_cropland_ssp5_2020 == 18] <- 1

plot(chen_cropland_ssp1_2020)
#plot(chen_cropland_ssp3_2020)
#plot(chen_cropland_ssp5_2020)
```
...
#cao_ssp3_2020_global <- rast("data/cao/globalCropland_2020CE_SSP3_RCP70.tif")
#cao_ssp5_2020_global <- rast("data/cao/globalCropland_2020CE_SSP5_RCP85.tif")
```
...
Cropland data for analysis will be defined as "Agriculture" from the MapBiomas classification, which includes temporary and perennial crops but omits poasture and mosaic agriculture.
...
mapbiomas_2020_resamp <- resample(mapbiomas_2020_agg, chen_cropland_ssp1_2020, method = "near")
...
```{r}
chen_mapbiomas <- mapbiomas_2020_reclass + chen_cropland_ssp1_2020
plot(chen_mapbiomas)

counts <- freq(chen_mapbiomas)
counts
