...
This markdown will be focusing on Brazil. 
...
Now that Mapbiomas data has been aggregated, resampled, and cropped, it's time to reclassify the raster to select for LULC categories of interest.
```{r}
# if above code chunk has been previously run, read in raster:
...
#writeRaster(mapbiomas_2020_reclass, "data/mapbiomas/mapbiomas_brasil_2020_cropland.tif")
```


Test the difference when including mosaic agriculture and paster:
```{r}
mapbiomas_2020_cropped <- rast("data/mapbiomas/mapbiomas_2020_cropped.tif")

# include ag mosaic in reclassification
...
# checking that crs of mapbiomas raster matches with brazil (desired crs)
plot(mapbiomas_crop_2020_mosaic)
plot(brazil, col = NA, add = TRUE)
...
Check how plot and confusion matrix change if mosaic ag and pasture is included
```{r}
# add chen and mapbiomas rasters 
...
# save intermediate raster to save on computing time in future
writeRaster(mapbiomas_2015_reclass, "data/mapbiomas/mapbiomas_brasil_2015_cropland.tif")

```

```{r}
# skip steps above (after run once) and read in saved raster below to save time
mapbiomas_2015_reclass <- rast("data/mapbiomas/mapbiomas_brasil_2015_cropland.tif") 

# crs do not match, so use terra::project() to match with chen raster
mapbiomas_crop_2015 <- project(mapbiomas_2015_reclass, crs(chen_crop_2015_brazil), method = "near")

# checking that crs of mapbiomas raster matches with brazil (desired crs)
plot(mapbiomas_crop_2015)
plot(brazil, col = NA, add = TRUE)
```
