...
## To greener pastures?
Quick check to see how much of the land classified as cropland is classified as pasture by Mapbiomas. Quick visual inspection in QGIS reveals that the majority seems to be pasture
```{r}
# code to turn all 0s in chen reclassified to NAs
# then mask mapbio by chen to remove all values in mapbio that aren't Chen cropland for 2020
# return values for mapbio and see the frequency of pasture
chen_crop_ssp1_2020_brazil_na <- chen_crop_ssp1_2020_brazil
chen_crop_ssp1_2020_brazil_na[chen_crop_ssp1_2020_brazil_na == 0] <- NA

plot(chen_crop_ssp1_2020_brazil_na)
...
# reclassify to remove values for cropland in mapbiomas
...
```
Yes, looks like the majority of mis-matched pixels are classified as either "pasture" (15) or "mosaic agriculture and pasture" (21) by Mapbiomas
